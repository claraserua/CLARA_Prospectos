...
$(document).ready(function(){
  ...
  if(gup("alert")){
    ...
    {
    case '1':
    $("#bodyAlerta").html("Se ha guardado el proyecto"); 
    break;
    case '2':
    $("#bodyAlerta").html("Se guardado en nuevo estado del proyecto"); 
    break;
	
	case '3':
    $("#bodyAlerta").html("La propuesta del proyecto se ha enviado"); 
    break;
	
	case '4':
    $("#bodyAlerta").html("La propuesta del proyecto se ha enviado exitosamente"); 
    break;
	
	case '5':
    $("#bodyAlerta").html("La propuesta del proyecto se ha enviado a la universidad para ajustes"); 
    break;
    
	case '6':
    $("#bodyAlerta").html("La propuesta fue revisada y ha pasado a ser integrada para su presentación"); 
    break;
	
	case '7':
    $("#bodyAlerta").html("La propuesta ha pasado exitosamente a estado de presentación de Propuesta"); 
    break;
	
	case '8':
    $("#bodyAlerta").html("La propuesta ha pasado exitosamente a Inicio de Proyecto"); 
    break;
    
	case '9':
    $("#bodyAlerta").html("La propuesta no ha sido aprobada"); 
	//$("#alerta").addClass("alert alert-success");
    break;
	
	case '10':
    $("#bodyAlerta").html("La propuesta ha pasado exitosamente a agendar presentación"); 
    break;
	//default:
...
$('#search_go-button-pj').click(function(){
	q = $('#searchbar').val();
...
function ImprimirProyecto(id){
	    var idproyecto = id;
	    var href = "?execute=proyectos/printproyecto&idProyecto="+idproyecto;
...
function EliminarProyecto(id,confirm){
      
	 if(!confirm){
	 $('#titlemodal').html('Eliminar Proyecto');
	 $('#bodymodal').html('¿Esta seguro de eliminar el proyecto');
	  $("#aceptarmodal").attr("onClick", "javascript:EliminarProyecto('"+id+"',true);return false;")
    ...
    buscar();
	  ...
	  $('html, body').animate({scrollTop:0}, 10);
                ...
                $("#alerta").addClass("alert alert-success");
				$("#headAlerta").html("¡Correcto!");
				$("#bodyAlerta").html("Se ha eliminado el proyecto"); 
	...
	/* setTimeout("window.location.href = '?execute=principal&Menu=F1&SubMenu=SF11&alert=1#&p=1&s=25&sort=1&q='",1000);*/
				...
				$("#bodyAlerta").html("<strong>No te alarmes</strong>!! al parecer no se elimino la propuesta del Proyecto correctamente, por favor intenta eliminarlo nuevamente."); 
