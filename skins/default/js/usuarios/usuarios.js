...
$('#search_go-button-usuarios').click(function(){
 ...
 function filtrar(cat){
  ...
  var categorias = new Array();
	...
	urltag = "&p=1&s="+gup('s')+"&sort="+gup('sort')+"&q="+encodeURI(gup('q'));
...
var s= parseInt(gup('s'));
  ...
  urltag = "&p="+gup('p')+"&s="+gup('s')+"&sort="+gup('sort')+"&q="+encodeURI(q);
   ...
   window.location.hash = urltag;
    ...
    type: "GET",  
    url: "index.php",  
    data: "execute=usuarios&method=Buscar"+urltag,  
...
function EliminarUsuario(id,confirm){
      
	 if(!confirm){
	 $('#titlemodal').html('Eliminar Usuario');
	 $('#bodymodal').html('¿Esta seguro de eliminar el usuario:<strong> '+id+'</strong>?');
	  $("#aceptarmodal").attr("onClick", "javascript:EliminarUsuario('"+id+"',true);return false;")
    ...
    type: "GET",  
    url: "index.php",  
    data: "execute=usuarios&method=Eliminar&id="+id,  
    success: function(msg){  
