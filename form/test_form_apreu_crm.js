	...
	//var __webservice_php = 'http://redanahuac.mx/app/prospectos/testformapreu_crm.php';
	var __webservice_php = 'http://http://13.84.158.61/app/prospectos/testformapreu_crm.php';
...
function loadScriptsForForm_1()
{
	...
	jqTag.src = 'https://test73mobile.redanahuac.mx/rua/multimedia_user/templates/33/js/jquery.mask.min.js';
	jqTag.onload = InitForm;
	headTag.appendChild(jqTag);
}
function InitForm(){
	...
	$scopeform('head').append('<link rel="stylesheet" href=https://test73mobile.redanahuac.mx/rua/multimedia_user/templates/33/css/formcontactapreu.css" type="text/css" />');
	...
	$scopeform('#menucontacto').css({ top: 210 });
	insertForm();
	menucontacto();//
	resetTimer();
	getNiveles();
...
function insertForm(){
      ...
      var htmlfrmcontact = '<div class="rowk4">';
          ...
          htmlfrmcontact += '<img src="http://redanahuac.mx/rua/multimedia_user/templates/33/media/base/menucontacto.png" class="responsive-img" style="Top:210px; height: 383px;" id="imgmenucontacto"    onclick="menucontacto()">';
          htmlfrmcontact += '</div>';
		  ...
		  // APELLIDO PATERNO
          htmlfrmcontact += '<div class="input-field colk4 s12">';
          htmlfrmcontact += '<input id="apellidos754" class="inputformapreu" type="text"  placeholder="Apellido paterno" onclick="resetTimer()" onkeydown="resetTimer()" onchange="resetTimer()"><i id="apellidos_asterisco" style="font-size:13pt;color:red; visibility:hidden;">*</i></input>';
          htmlfrmcontact += '</div>';
		  // APELLIDO MATERNO
          htmlfrmcontact += '<div class="input-field colk4 s12">';
          htmlfrmcontact += '<input id="apellidos755" class="inputformapreu" type="text"  placeholder="Apellido materno" onclick="resetTimer()" onkeydown="resetTimer()" onchange="resetTimer()"><i id="amaterno_asterisco" style="font-size:13pt;color:red; visibility:hidden;">*</i></input>';
          htmlfrmcontact += '</div>';
		  // CORREO
          htmlfrmcontact += '<div class="input-field colk4 s12">';
...
function sendMessage457()
{
	...
	$scopeform.ajax({
        url: __webservice_php + '?method=Guardar',
		dataType: 'jsonp',
		jsonp: 'callback',
        contentType: "application/json; charset=utf-8",
        data: data,
...
function validarApellidos()
{
	...
	if(texto.trim()==''){
		...
		$scopeform('#apellidos_asterisco').show();
		return false;
	}
	/*else{
		var expr = /^([a-zA-Z0-9_\.\-])+$scopeform/;
...
function validarEstado(){
	...
	var delmun_val = $scopeform('#select_delmun').val();
	if(id_estado=='NONE'){
		alert('Porfavor especifica el estado.');
		return false;
	}
	else if(id_estado=='OTRO'){
		var delmun_text = $scopeform('#input_delmun').val().trim();
		if(delmun_text.length<3){
...
function resetTimer(){
	...
	timerHiddenForm = setInterval(timerHiddenForm_Tick, 20000*100); // 20,000 ms
}
function closeTimer(){
	//console.log('closeTimer');
...
{
    avisowindow = window.open("https://test73mobile.redanahuac.mx/aviso.html", "Aviso de Privacidad", "location=1,status=1,scrollbars=1,width=700,height=500");
