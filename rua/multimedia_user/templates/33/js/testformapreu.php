 ...
 function Guardar(){	
	...
	function EnviarCRM()
	{
		echo $_GET['callback']."('";
			...
			if($idnivel==3 || $idnivel==4 || $idnivel==5 || $idnivel==6){  //Maestrías en línea,  Maestrías presenciales, Doctorados y especialidades
			}
			else if($campus != 'UAT'){//2 Licenciaturas
...
<img src="http://testprospectos.redanahuac.mx/skins/default/img/logo_anahuac.png"  alt="Red de Universidades Anahuac" border="0">
...
function getEstados(){
	...
	$sql = "SELECT DISTINCT(d_estado) AS 'ESTADO', c_estado AS 'ID_ESTADO' FROM SEPOMEX ORDER BY c_estado";
	$rows = database::getRows($sql);
	...
	foreach($rows as $row){
		...
		$html.= "<option value=\"$ID_ESTADO\">$ESTADO</option>";
	}
