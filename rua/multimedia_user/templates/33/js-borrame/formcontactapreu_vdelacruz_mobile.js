...
if (xhrUNO.readyState == 4 && xhrUNO.status == 200) {
		...
		es_produccion = inArray(window.location.host,response); 
		 ...
		 if (es_pruebas)
			{
			__webservice_php = 'https://test.redanahuac.mx/testformapreu_crm_vdelacruz.php';
			__dominio =        'https://test.redanahuac.mx';
			__url_home =        'https://test.anahuac.mx';
			__url_aviso_prepa = 'https://test.anahuac.mx/prepa/gracias';
		...
		__url_aviso_prepa = 'http://anahuac.mx/prepa/gracias';
...
if (xhrDOS.readyState == 4 && xhrDOS.status == 200) {
		...
		__url_home =        'https://anahuac.mx';
		__url_aviso_prepa = 'http://anahuac.mx/prepa/gracias';
		__webservice_php_leads = 'http://leads.redanahuac.mx/api/leads';
...
xhrUNO.open('GET', "https://redanahuac.mx/app/prospectos/checadominioproduccion.php", true);
xhrUNO.send();

xhrUNO.onreadystatechange = processRequestUNO;

xhrDOS.open('GET', "https://redanahuac.mx/app/prospectos/checadominiopruebas.php", true);
xhrDOS.send();
...
      modalrua.style.display = 'none';
	...
	$scopeform( ".close" ).click( function() {
	//window.location.reload(__url_home);
	modalrua.style.display = "none";
	});
	
//$scopeform( "#buttonSend" ).click( function() {
	$scopeform( "#buttonSend" ).unbind('click').click(function() {
//$scopeform( "#buttonSend" ).one('click', function (event) {  
		event.preventDefault();	
		...
		
		{
		   ...
		   sendMessage457();	
		   $scopeform( "#buttonSend" ).prop( "disabled", true );				
		}
...
function insertForm(){
      ...
      var htmlfrmcontactmodal = '<!-- The Modal -->';
          ...
          //htmlfrmcontactmodal += '<span class="close">×</span>';
		   htmlfrmcontactmodal += '<span class="close" onclick="window.location.reload('+__url_home +'); return false;">×</span>';
...
// APELLIDO PATERNO, MATERNO
  ...
  htmlfrmcontact += '<div style="height: 20px;"></div>';
		  ...
		  htmlfrmcontact += '<input type="button" value="Enviar" id="buttonSend" class="waves-effect98 waves-light5645 btn9796 orange0976" style="font-size: 1rem;">';
          htmlfrmcontact += '</div>';
...
//alert(msg);
         ...
         console.log("getProgramsPos="+msg);
...
function checaduplicado(data)
{
//var url_webservice = __webservice_php + '?method=ChecaDuplicado';
var s ='NO';
  $scopeform.ajax({
        url: __webservice_php + '?method=ChecaDuplicado',
       dataType: 'jsonp',
      // dataType: 'text',
        jsonp: 'callback',
        contentType: "application/json; charset=utf-8",
        ...
        error: function (xhr, status, errorThrown) {
            if(console)
            {
              console.log('duplicado err='+xhr.status+' desc='+    xhr.responseText);
            }
...
function sendMessage457()
{
  ...
  /* vdelacruz */
	...
	var campus = $scopeform('#campusrua854').val();
  var campustexto = $scopeform('#campusrua854 option:selected').text();
...
if (data.nivel == 3)
{
 // sendToLeads(data);
  ...
  if(campustexto.slice(-1)!='*')
  {
    if(checaduplicado(data)=='NO')
    {
    //  alert('duplicado');
    }
		...
		//	buttonSend_finish();
			...
			if (!isNaN(idprospecto) && idprospecto > 0){
				...
				data.idprospecto=idprospecto;
					  if(checaduplicado(data)=='NO' && !esProgramaParcial())
						{
							sendToCRM(data);
						//console.log( "envia a crm");
						}
...
function limpia()
{
	//location.reload();
			...
			$scopeform("#campusrua854").html('<option value="NONE" >Campus de Inter&eacute;s ...</option>');
			$scopeform("#campusrua854").prop('disabled', true);
            $scopeform("#periodorua6855").html('<option value="NONE">Período de interés</option>' + per);
			...
			$scopeform('#input_delmun').val('');
            $scopeform('#select_sexo').val("NONE");
			$scopeform('#select_dia').val("NONE");
			$scopeform('#select_mes').val("NONE");
			$scopeform('#select_anio').val("NONE");
			
			$scopeform('#select_estado').val('NONE');
			$scopeform('#select_delmun').html('<option value="NONE" >Delegaci&oacute;n o municipio ...</option>');
			$scopeform('#select_delmun').show();
			$scopeform('#input_delmun').val('');
...
{
	var dataleads= [];
	dataleads['LProgramCode']= data['codigo'];
...
/*function IniciarSesion(){
	...
	var url = 'http://test.redanahuac.mx/mobile/webservice/curl.php';
