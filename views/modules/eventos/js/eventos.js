...
var s= parseInt(gup('s'));
  ...
  urltag = "&p="+gup('p')+"&s="+gup('s')+"&sort="+gup('sort')+"&q="+encodeURI(q);
   ...
   window.location.hash = urltag;
    ...
    type: "GET",  
    url: "index.php",  
    data: "execute=eventos&method=Buscar"+urltag,  
	...
	switch(s)
{
 ...
 function AsignarPlanEstrategico(confirm){
  ...
  function AgregarNoticia(){
    ...
    type: "POST",  
    url: "index.php?execute=eventos&method=Guardar",  
    data: { programa: programa, grado: grado,nombre: nombre, apellidos: apellidos, telefono: telefono, correo: correo, ciudad: ciudad},
    success: function(msg){  
	//alert(msg);
...
var arr = $("input:checked.usrcheck").getCheckboxValues();
 ...
 if(arr.length==0){
	...
	//alert(arr); // esto muestra un pop-up con los checkboxes seleccionados	
	 ...
	 setTimeout("window.location.href = '?execute=descargar_prospecto_posgrado&usuarios="+usuarios+"'",0);
...
function EnviarCorreo(confirm)
{
	...
	{*/
		...
		else
		{
		 ...
		 // alert(usuarios);
			...
			$.ajax({	 
		    type: "POST",  
		    url: "index.php?execute=eventos&method=EnviarCorreoProsp",  
  ...
  function BuscarProspecto(idprospecto){  	
    ...
    type: "POST",  
    url: "index.php?execute=eventos&method=BuscarProspecto",  
	...
	//  $('#campus option[value="'+contenido[0]+'"]').attr('selected', 'selected');
	  $('#programa option[value="'+contenido[0]+'"]').attr('selected', 'selected');
	  $('#nombre').val(contenido[1]);
	  
	  $('#apellidos').val(contenido[2]);  
	  $('#telefono').val(contenido[3]);
	  $('#correo').val(contenido[4]);
	  $('#ciudad').val(contenido[5]);
  ...
  function EditarProspecto(idprospecto){
    ...
    type: "POST",  
    url: "index.php?execute=eventos&method=Editar",  
    data: { idprospecto:idprospecto,programa: programa, grado: grado,nombre: nombre, apellidos: apellidos, telefono: telefono, correo: correo, ciudad: ciudad},
    success: function(msg){  
	//alert(msg);
...
var arr = $("input:checked.usrcheck").getCheckboxValues();
 ...
 if(arr.length==0){
  ...
  function EliminarUsuario(confirm){
	 ...
	 $('#bodymodal').html('¿Esta seguro de eliminar el usuario?');
	...
	$.ajax({	 
    type: "POST",  
    url: "index.php?execute=eventos&method=Eliminar",  
    data: { usuarios:usuarios},
    success: function(msg){  
...
function EliminarNoticia(id,confirm){
      
	  $("#alerta").hide();
	  var idevento = id;
	 if(!confirm){
	 $('#titlemodal').html('Eliminar Noticia');
	 $('#bodymodal').html('¿Esta seguro de eliminar la Noticia? Se eliminaran todos sus objetivos');
	  $("#aceptarmodal").attr("onClick", "javascript:EliminarNoticia('"+id+"',true);return false;")
	 
     $('#myModal').modal('show');
	 }else{
	 $('#myModal').modal('hide');
	 			 
	 $.getJSON('https://prospectos.redanahuac.mx/app/mobile/WSGetNoticias.php?callback=?','evento=deletenoticia&id='+idevento+'',function(res){
  		   ...
  		   $("#alerta").fadeIn();
                ...
                $("#alerta").addClass("alert alert-success");
				$("#headAlerta").html("¡Correcto!");
				$("#bodyAlerta").html("Se ha eliminado correctamente la noticia");
  		
  		 $('#noticia'+idevento).slideUp(1000);
