<?php
include "models/fichas.model.php";
...
class fichas {
	...
	function __construct() {
	 ...
	 function loadContent(){
	  ...
	  $section = TEMPLATE.'modules/fichas/GRDFICHAS.TPL';
	  $section = $this->View->loadSection($section);
	  ...
	  function Buscar(){
		...
		foreach($this->Model->fichas as $row){
			 ...
			 $recurso .= '<tr>
								...
								<td><h3>'.__toHtml($row['DESCRIPCION'], ENT_QUOTES, "ISO-8859-1").'</h3></td>
								<td><a href="?execute=fichas/editfichas&method=default&Menu=F3&SubMenu=SF33&IDFicha='.$row['PK1'].'"><button class="btn btn-small"><i class="icon-pencil"></i> Editar</button></a></td>
							</tr>';
			}	
