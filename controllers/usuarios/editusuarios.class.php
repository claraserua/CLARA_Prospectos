...
class editusuarios {
	...
	function error(){
	 ...
	 function loadContent(){
		...
		$contenido = $this->View->replace('/\#NOMBRE\#/ms' ,__toHtml($row['NOMBRE'], ENT_QUOTES, "ISO-8859-1"),$contenido);
		$contenido = $this->View->replace('/\#APELLIDOS\#/ms' ,__toHtml($row['APELLIDOS'], ENT_QUOTES, "ISO-8859-1"),$contenido);
		$contenido = $this->View->replace('/\#CORREO\#/ms' ,__toHtml($row['EMAIL'], ENT_QUOTES, "ISO-8859-1"),$contenido);
		...
		foreach($this->Model->roles as $row){
		    ...
		    if(!in_array($row['PK1'], $this->Model->rolesusuario)){
			$roles .= '<option value="'.$row['PK1'].'">'.__toHtml($row['ROLE'], ENT_QUOTES, "ISO-8859-1").'</option>';
			  }else{
			$rolesusuario .= '<option value="'.$row['PK1'].'">'.__toHtml($row['ROLE'], ENT_QUOTES, "ISO-8859-1").'</option>';
			  }
			}
