...
class jerarquia {
	...
	$this->View->replace_content('/\#NODOS\#/ms' ,$nodos);
	 ...
	 function loadContent(){
	   ...
	   $section = $this->View->replace('/\#NIVEL\#/ms' ,__toHtml($row['NOMBRE'], ENT_QUOTES, "ISO-8859-1"),$section);
	 ...
	 function Buscar(){
		...
		$recurso .= '<tr>
		    <td><h3>'.$row['PK1'].'</h3></td>
			<td><h3><a href="?execute=jerarquia&method=default&Menu=F3&SubMenu=SF34&Nivel='.$row['PK1'].'#&p=1&s=25&sort=1&q=">'.__toHtml($row['NOMBRE'], ENT_QUOTES, "ISO-8859-1").'</a></h3></td>
			<td><h3><a href="?execute=jerarquia&method=default&Menu=F3&SubMenu=SF34&Nivel='.$row['PK1'].'#&p=1&s=25&sort=1&q=">'.__toHtml($row['DESCRIPCION'], ENT_QUOTES, "ISO-8859-1").'</a></h3></td>
		...
		$recurso =$this->getPaginadoHeader();
		$recurso .= "#%#";
		$recurso .= '<tr> <td colspan="4"><div class="empty_results">El nivel no contiene subniveles</div></td></tr>';
		$recurso .= "#%#";
		$recurso .=$this->getpaginadoFooter();
		$recurso .= "#%#";
		$recurso .= $total;
