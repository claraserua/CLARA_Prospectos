<?php
include "models/proyectos/agendarproyecto.model.php";
include "libs/resizeimage/class.upload.php";


class agendarproyecto {
	...
	function __construct() {
	 ...
	 $this->Model = new agendarProyectoModel();
	 ...
	 switch($_GET['method']){
		...
		case "EnviarPresentacionPropuesta":
	        $this->EnviarPresentacionPropuesta();
			break;	
	...
	function error(){
	 ...
	 function loadContent(){
		...
		$contenido = $this->View->Template(TEMPLATE.'modules/proyecto/AGENDARPROYECTO.TPL');
		...
		if($row){
			...
			$estado = '<span class="label label-warning">Agendar Presentación</span>';
		}	
		...
		if($this->passport->getPrivilegio($idProyecto,'P312')){
		
	    $btnEnviar ='<button class="btn btn-large btn-primary" onclick="EnviarPresentacionPropuesta(false);"><span class="icon icon-white icon-sent"></span> Presentación</button>';
 ...
 function insertarComentario(){
   ...
   $panelcontent .=' </div>
	  ...
	  function EnviarPresentacionPropuesta(){		
			 ...
			 $idProyecto = $_GET['idProyecto'];	
			 $this->Model->EnviarPresentacionPropuesta($idProyecto,$_GET['estado']);
