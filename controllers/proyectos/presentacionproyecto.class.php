<?php
include "models/proyectos/presentacionproyecto.model.php";
include "libs/resizeimage/class.upload.php";


class presentacionproyecto {
	...
	//  $this->nodoprincipal = new Niveles("option");	 
	 $this->Model = new presentacionProyectoModel();
	 ...
	 switch($_GET['method']){
		...
		case "insertarComentarioGral": 		   
		 ...
		 case "EnviarResolucionFavorable":
	        $this->EnviarResolucionFavorable();
			break;	
			
		case "EnviarResolucionFavorableAjustes":
	        $this->EnviarResolucionFavorableAjustes();
			break;	
			
		case "EnviarResolucionNoFavorable":
	        $this->EnviarResolucionNoFavorable();
			break;	
	...
	function error(){
	 ...
	 function loadContent(){
		...
		$contenido = $this->View->Template(TEMPLATE.'modules/proyecto/PRESENTACIONPROYECTO.TPL');
		...
		if($row){
			...
			$estado = '<span class="label label-warning">Presentación de Propuesta</span>';
		}	
		...
		if($this->passport->getPrivilegio($idProyecto,'P313')){
		
	     $btnResolucion ='<button class="btn btn-large btn-primary" onclick="EnviarResolucionFavorable();"><span class="icon icon-white icon-sent"></span> Resolución Favorable</button>';
		
		$contenido =  $this->View->replace('/\<!--#BTNRENVIARRESOLUCION#-->/ms' ,$btnResolucion,$contenido);		
		}				
		
	
				
		if($this->passport->getPrivilegio($idProyecto,'P314')){
			
	    $btnResolFavorable ='<button class="btn btn-large btn-primary" onclick="EnviarResolucionFavorableAjustes();"><span class="icon icon-white icon-sent"></span> Resolución favorable con ajuste</button>';
		
		$contenido =  $this->View->replace('/\<!--#BTNRENVIARRESOFAVORABLE#-->/ms' ,$btnResolFavorable,$contenido);	
		
		}
		
		if($this->passport->getPrivilegio($idProyecto,'P315')){
		
	    $btnResolNoFavorable ='<button class="btn btn-large btn-primary" onclick="EnviarResolucionNoFavorable(false);"><span class="icon icon-white icon-sent"></span> Resolución no favorable</button>';
		
		$contenido =  $this->View->replace('/\<!--#BTNRENVIARRESONOFAVORABLE#-->/ms' ,$btnResolNoFavorable,$contenido);	
		}
	    ...
	    $btnAdjuntar ='<div class="box">
					                <!-- Box Head -->
					                 <div class="box-head" style="height:60px;">
									 <p> Documentos que apoyan la evaluación del proyecto				
									 </p>
 ...
 function insertarComentario(){
   ...
   $panelcontent .=' </div>
	 ...
	 function EnviarResolucionFavorable(){		
			 ...
			 $idProyecto = $_GET['idProyecto'];	
			 $this->Model->EnviarResolucionFavorable($idProyecto,$_GET['estado']);		
			
		  }
		  
	 function EnviarResolucionFavorableAjustes(){
		  	
			$this->Model->EnviarResolucionFavorableAjustes($_GET['idProyecto'],$_GET['estado']);			
			
		  }	 
	 function EnviarResolucionNoFavorable(){
		  	
			$this->Model->EnviarResolucionNoFavorable($_GET['idProyecto'],$_GET['estado']);
