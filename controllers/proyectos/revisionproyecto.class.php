<?php
include "models/proyectos/revisionproyecto.model.php";
include "libs/resizeimage/class.upload.php";


class revisionproyecto {
	...
	function __construct() {
	 ...
	 $this->Model = new revisionProyectoModel();
	...
	function error(){
	 ...
	 function loadContent(){
		...
		$contenido = $this->View->Template(TEMPLATE.'modules/proyecto/REVISIONPROYECTO.TPL');
		...
		if($row){
			...
			$estado = '<span class="label label-warning">Revisión</span>';
		}	
		...
		if($this->passport->getPrivilegio($idProyecto,'P305')){
		
	    $btnEnviar ='<button class="btn btn-large btn-primary" onclick="EnviarConsultaExpertos(false);"><span class="icon icon-white icon-sent"></span> Enviar consulta a expertos</button>';
	...
	//	if($this->passport->getPrivilegio($idProyecto,'P293')){
	    ...
	    $btnAdjuntar ='<div class="box">
					                <!-- Box Head -->
					                 <div class="box-head" style="height:60px;">
									 <p>Documentos que apoyen la evaluación del proyecto
