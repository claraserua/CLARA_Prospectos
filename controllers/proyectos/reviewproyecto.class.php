<?php
include "models/proyectos/reviewproyecto.model.php";
include "libs/resizeimage/class.upload.php";


class reviewproyecto {
	...
	function __construct() {
	 ...
	 $this->Model = new reviewProyectoModel();
	 ...
	 switch($_GET['method']){
		...
		case "insertarComentarioGral": 		   
		 ...
		 case "RegresarPropUni":
	        $this->RegresarPropUni();
			break;	
			
		case "EnviarConRevisionPreliminar":
	        $this->EnviarConRevisionPreliminar();
			break;	
	...
	function error(){
	 ...
	 function loadContent(){
		...
		$contenido = $this->View->Template(TEMPLATE.'modules/proyecto/REVIEWPROYECTO.TPL');
	...
	/*	if($this->passport->getPrivilegio($idProyecto,'P245')){
		...
		if($this->passport->getPrivilegio($idProyecto,'P244')){
			
	    $btnRegresar ='<button class="btn btn-large btn-primary" onclick="RegresarPropUni();"><span class="icon icon-white icon-sent"></span> Regresar Propuesta</button>';
		
		$contenido =  $this->View->replace('/\<!--#BTNREGRESARIARPROP#-->/ms' ,$btnRegresar,$contenido);	
		
		}
		
		if($this->passport->getPrivilegio($idProyecto,'P302')){
		
	    $btnEnviar ='<button class="btn btn-large btn-primary" onclick="EnviarConRevisionPreliminar(false);"><span class="icon icon-white icon-sent"></span> Enviar propuesta</button>';
		...
		if($this->passport->getPrivilegio($idProyecto,'P256')){			
			...
			$permisoborrar='P288';	
	       	$contenido =  $this->View->replace('/\#NR\#/ms' ,$this->Model->getNumComentariosGralesProyecto($idProyecto),$contenido);
				...
				$contenido =  $this->View->replace('/\<!--#COMENTARIOSTABCOMGRALES#-->/ms' ,$this->getComentariosGralesProyecto($idProyecto,$permisocomentar,$permisoborrar),$contenido);	
 ...
 function insertarComentario(){
   ...
   $panelcontent .=' </div>
	 ...
	 function RegresarPropUni(){		
			 ...
			 $idProyecto = $_GET['idProyecto'];	
			 $this->Model->RegresarPropUni($idProyecto,$_GET['estado']);
		
			
		  }
		  
	 function EnviarConRevisionPreliminar(){
		  	
			$this->Model->EnviarConRevisionPreliminar($_GET['idProyecto'],$_GET['estado']);
