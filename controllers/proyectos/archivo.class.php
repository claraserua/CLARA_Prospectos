<?php
include "models/proyectos/archivo.model.php";
...
class archivo {
	...
	function viewRecurso(){
	    ...
	    $row = $this->Model->getRecurso($_GET['idArchivo']);//es igual que getArchivo de addProyectoModel
		$titulo = __toHtml($row['TITULO']);
		...
		$preview = "<img src='".$IMAGEN170X170."'/>";
			
		}
		
		//$preview = "<img src='media/planesoperativos/evidencias/".$imagen."'/>";
		//$autor = $row['AUTOR'];
		$formato = "";
...
WIDTH="340" HEIGHT="220" ShowControls="1" ShowStatusBar="0" ShowDisplay="0" autostart="0"> </EMBED>
	  ...
	  case "TXT":
	   ...
	   break;
		...
		$this->View->template = TEMPLATE.'ARCHIVO.TPL';
		...
		//$this->View->replace_content('/\#AUTOR\#/ms' ,__toHtml($autor));
		...
		$this->View->replace_content('/\#FECHA\#/ms' ,$row['FECHA_R']);
		
		
		/* $permiso =  ( $this->Model->obtenerEstadoPlan($_GET['IDPlan'])=="R" )? "P136":"P64";
	     if($this->passport->getPrivilegio($_GET['IDPlan'],$permiso)){*/
	   	
		   $btndescargar = '<input  type="button" class="btn btn-warning" onclick="#LINKDESCARGA#" value="Descargar"/>';
		 /*}else{
		 	$btndescargar = " ";
		 }*/
	  ...
	  function insertarComentario(){
			...
			$id = $this->Model->insertarComentario($_POST['comentario'],$_POST['idArchivo']);
	...
	<strong><a href="#">'.__toHtml($usuario).'</a></strong><br/>
    '.$_POST['comentario'].'
	 ...
	 $panelcontent ="";
	  $this->Model->getComentarios($_GET['idArchivo']);
		   ...
		   //}
			 ...
			 $panelcontent .= '<strong><a href="#" class="comentuser">'.__toHtml($usuario).'</a></strong><br/>
						'.$comentario.'
