<?php
include "models/proyectos/asignaciones.model.php";
...
class asignaciones {
	...
	function __construct() {
	 ...
	 function loadContent(){
	  ...
	  $section = TEMPLATE.'modules/proyecto/GRDASIGNACIONES.TPL';
	  $section = $this->View->loadSection($section);
	  
	   $urlbtnaddusuario ='<a href="?execute=proyectos/asignarusuario&method=default&Menu=F1&SubMenu=SF11&IDProyecto='.$_GET['IDProyecto'].'&IDJerarquia='.$_GET['IDJerarquia'].'#&p=1&s=25&sort=1&q="><button type="submit" class="btn btn-small btn-warning"><i class="icon-user icon-white"></i>&nbsp;Asignar Usuario</button></a>';
	...
	function Buscar(){
		...
		$imagen = ($row['IMAGEN'] == "avatar.jpg") ? "user.png" : "thum_40x40_".$row['IMAGEN'];
			 ...
			 $recurso .= '<tr '.$clase.'>
								...
								<td align="center"><h3>'.__toHtml($this->Model->getTipoRol($row['ROL']), ENT_QUOTES, "ISO-8859-1").'</h3></td>
								<td>
                                <h3><strong>'.__toHtml($this->Model->getRol($row['ROL']), ENT_QUOTES, "ISO-8859-1").'</strong></h3>				
								</td>
