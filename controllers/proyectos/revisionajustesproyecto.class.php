<?php
include "models/proyectos/revisionajustesproyecto.model.php";
include "libs/resizeimage/class.upload.php";


class revisionajustesproyecto {
	...
	function __construct() {
	 ...
	 $this->Model = new revisionajustesProyectoModel();
	 ...
	 switch($_GET['method']){
		...
		case "EnviarConsultaExpertos":
	        $this->EnviarConsultaExpertos();
			break;	
			
		 case "EnviarIntegrarPropuesta":
	        $this->EnviarIntegrarPropuesta();
			break;	
			
		case "EnviarSolicitudAjustes":
	        $this->EnviarSolicitudAjustes();
			break;	
	...
	function error(){
	 ...
	 function loadContent(){
		...
		$contenido = $this->View->Template(TEMPLATE.'modules/proyecto/REVISIONAJUSTESPROYECTO.TPL');
		...
		if($row){
			...
			$estado = '<span class="label label-warning">Revisión Ajustes</span>';
		}	
		...
		if($this->passport->getPrivilegio($idProyecto,'P335')){
			
	    $btnConsultar ='<button class="btn btn-large btn-primary" onclick="EnviarConsultaExpertos();"><span class="icon icon-white icon-sent"></span> Consultar Expertos</button>';
		
		$contenido =  $this->View->replace('/\<!--#BTNRCONSULTAREXPERTOS#-->/ms' ,$btnConsultar,$contenido);	    }
		
		
		if($this->passport->getPrivilegio($idProyecto,'P319')){
		
	     $btnIntegracion ='<button class="btn btn-large btn-primary" onclick="EnviarIntegrarPropuesta();"><span class="icon icon-white icon-sent"> </span> Integrar propuesta</button>';
		
		$contenido =  $this->View->replace('/\<!--#BTNRENVIARINTEGRAR#-->/ms' ,$btnIntegracion,$contenido);		
		}				
		
	
				
		if($this->passport->getPrivilegio($idProyecto,'P321')){
			
	    $btnSolicitud ='<button class="btn btn-large btn-primary" onclick="EnviarSolicitudAjustes();"><span class="icon icon-white icon-sent"></span> Ajustes a la Universidad</button>';
		
		$contenido =  $this->View->replace('/\<!--#BTNRENVIARSOLICITUD#-->/ms' ,$btnSolicitud,$contenido);	
 ...
 function insertarComentario(){
   ...
   $panelcontent .=' </div>
	  ...
	  function EnviarConsultaExpertos(){		
			 ...
			 $idProyecto = $_GET['idProyecto'];	
			 $this->Model->EnviarConsultaExpertos($idProyecto,$_GET['estado']);		
	 ...
	 function EnviarIntegrarPropuesta(){		
			 ...
			 $idProyecto = $_GET['idProyecto'];	
			 $this->Model->EnviarIntegrarPropuesta($idProyecto,$_GET['estado']);		
			
		  }
		  
	 function EnviarSolicitudAjustes(){
		  	
			$this->Model->EnviarSolicitudAjustes($_GET['idProyecto'],$_GET['estado']);			
