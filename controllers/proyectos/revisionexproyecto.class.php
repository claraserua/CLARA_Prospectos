<?php
include "models/proyectos/revisionexproyecto.model.php";
include "libs/resizeimage/class.upload.php";


class revisionexproyecto {
	...
	function __construct() {
	 ...
	 $this->Model = new revisionexProyectoModel();
	 ...
	 switch($_GET['method']){
		...
		case "EnviarRevisadoExpertos":
	        $this->EnviarRevisadoExpertos();
			break;
			
		case "EnviarComentariosExpertos":
	        $this->EnviarComentariosExpertos();
			break;	
	...
	function error(){
	 ...
	 function loadContent(){
		...
		$contenido = $this->View->Template(TEMPLATE.'modules/proyecto/REVISIONEXPROYECTO.TPL');
		
		if($row){
			
			$idProyecto = $row['PK1'];						
			$estado = '<span class="label label-warning">Revisión</span> <span class="label label-info">expertos</span>';
		}	
		...
		if($this->passport->getPrivilegio($idProyecto,'P306')){
		
	    $btnComentar ='<button class="btn btn-large btn-primary" onclick="EnviarComentariosExpertos(false);"><span class="icon icon-white icon-sent"></span> Enviar comentarios</button>';
		
		$contenido =  $this->View->replace('/\<!--#BTNENVIARPROP#-->/ms' ,$btnComentar,$contenido);	
		}
		
		
		
		if($this->passport->getPrivilegio($idProyecto,'P336')){
			
		$btnEnviar ='<button class="btn btn-large btn-primary" onclick="EnviarRevisadoExpertos(false);"><span class="icon icon-white icon-sent"></span> Enviar revisión</button>';
		
		$contenido =  $this->View->replace('/\<!--#BTNENVIAREVISION#-->/ms' ,$btnEnviar,$contenido);	
		}	
	...
	//	if($this->passport->getPrivilegio($idProyecto,'P256')){			
			...
			$permisoborrar='P288';	
	       	$contenido =  $this->View->replace('/\#NR\#/ms' ,$this->Model->getNumComentariosGralesProyecto($idProyecto),$contenido);
				...
				$contenido =  $this->View->replace('/\<!--#COMENTARIOSGENERALESTABCOMENTRALES#-->/ms' ,$this->getComentariosGralesProyecto($idProyecto,$permisocomentar,$permisoborrar),$contenido);	
 ...
 function insertarComentario(){
   ...
   $panelcontent .=' </div>
		  ...
		  function eliminarComentarioGralProyecto(){
		   ...
		   function EnviarComentariosExpertos(){		
			
			 $idProyecto = $_GET['idProyecto'];	
			 $this->Model->EnviarComentariosExpertos($idProyecto,$_GET['estado']);
		
			
		  }
		  
		   function EnviarRevisadoExpertos(){		
			
			 $idProyecto = $_GET['idProyecto'];	
			 $this->Model->EnviarRevisadoExpertos($idProyecto,$_GET['estado']);
