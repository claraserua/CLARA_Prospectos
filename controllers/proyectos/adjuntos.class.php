<?php
include "models/planestrategico/adjuntos.model.php";
...
class adjuntos {
	...
	$this->View->replace_content('/\#NODOS\#/ms' ,$nodos);
	 ...
	 function loadContent(){
	  ...
	  $section = TEMPLATE.'modules/planestrategico/ADJUNTOS.TPL';
	  $section = $this->View->loadSection($section);
	  
	   $row = $this->getPlan();
	   $section = $this->View->replace('/\#TITULO\#/ms' ,__toHtml($row['TITULO']),$section);
	   ...
	   $urlupload = '?execute=planestrategico/uploadPlan&method=default&Menu=F1&SubMenu=SF11&IDPlan='.$row['PK1'].'';
	 ...
	 function Buscar(){
		...
		foreach($this->Model->archivos as $row){
		    ...
		    if(trim($row['TIPO'])=="IMG"){
			  $imagen = "media/planesestrategicos/thum_170x170_".$row['IMAGEN'];
			}else{
				$imagen = $row['IMAGEN'];
			}
