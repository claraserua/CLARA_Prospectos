<?php
include "models/proyectos/integrarproyecto.model.php";
include "libs/resizeimage/class.upload.php";


class integrarproyecto {
	...
	function __construct() {
	 ...
	 $this->Model = new integrarProyectoModel();
	 ...
	 switch($_GET['method']){
		...
		case "EnviarAgendarPropuesta":
	        $this->EnviarAgendarPropuesta();
			break;	
	...
	function error(){
	 ...
	 function loadContent(){
		...
		$contenido = $this->View->Template(TEMPLATE.'modules/proyecto/INTEGRARPROYECTO.TPL');
		...
		if($row){
			...
			$estado = '<span class="label label-warning">Integrar propuesta</span>';
		}	
		...
		if($this->passport->getPrivilegio($idProyecto,'P338')){
		
	    $btnEnviar ='<button class="btn btn-large btn-primary" onclick="EnviarAgendarPropuesta(false);"><span class="icon icon-white icon-sent"></span> Agendar propuesta</button>';
 ...
 function insertarComentario(){
   ...
   $panelcontent .=' </div>
	  ...
	  function EnviarAgendarPropuesta(){		
			 ...
			 $idProyecto = $_GET['idProyecto'];	
			 $this->Model->EnviarAgendarPropuesta($idProyecto,$_GET['estado']);
