...
class editplanelineas {
	...
	function error(){
	 ...
	 function loadContent(){
		...
		$row = $this->getPlan();
		$section = $this->View->replace('/\#TITULO\#/ms' ,__toHtml($row['TITULO'], ENT_QUOTES, "ISO-8859-1"),$section);
		...
		foreach($this->Model->lineas as $row){
			...
			$panelcontent .= '<div id="linea'.$cont.'" class="box" style="display: block;">
						...
						<textarea style="width:700px;" class="input-xlarge focused" id="titulo'.$cont.'" name="titulo" >'.__toHtml($linea, ENT_QUOTES, "ISO-8859-1").'</textarea>
...
<textarea style="width:700px;" name="L'.$cont.'-objetivo" id="L'.$cont.'-objetivo'.$contobjetivo.'" >'.__toHtml($rowObjE['OBJETIVO'], ENT_QUOTES, "ISO-8859-1").'</textarea>
