...
class categorias {
	...
	function __construct() {
	 ...
	 function loadPage(){
		...
		$this->View->template = TEMPLATE.'/modules/categorias/CATEGORIAS.TPL';	
		$this->View->loadTemplate();
		$this->loadHeader();
		$this->loadNodos();
		$this->loadMenu();
		 if($this->passport->privilegios->hasPrivilege('P02')){
		$this->loadContent();
		}else{
		$this->error();
		}
	...
	function error(){
	 ...
	 $section = TEMPLATE.'modules/categorias/GRDCATEGORIAS.TPL';
	 $section = $this->View->loadSection($section);
	 
	  if($this->passport->privilegios->hasPrivilege('P66')){
	 $urlbtnaddplan ='<button type="button" onclick="AsignarPlanEstrategico(false);" class="btn btn-small btn-warning"><i class="icon-book icon-white"></i>&nbsp;Agregar nueva categoría</button>';
	  }else{
	  	$urlbtnaddplan ='';
	  }
	 ...
	 function Buscar(){
	   ...
	   $urlEditObjetivos="?execute=planesoperativo/editobjetivos&method=default&Menu=F2&SubMenu=SF21&IDPlan=".$row['PK1']."&IDPlanE=".$row['PK_PESTRATEGICO']."&IDJerarquia=".$row['PK_JERARQUIA'];
			...
			$recurso .= '<tr '.$clase.'>
								...
								<a href="'.$url.'"><span  style="float:left; margin-top:8px;"><h3>'.__toHtml($row['TITULO']).'</h3></span></a>
								...
								$recurso .='<li><a href="#" onClick="javascript:ImprimirPlan(\''.$row['PK1'].'\',\''.$row['PK_PESTRATEGICO'].'\');return false;"><i class=" icon-print"></i> Imprimir</a></li>';
								}
								 
								 
								if($this->passport->getPrivilegio($row['PK1'],'P28')){
								$recurso .='  <li><a href="?execute=planesoperativo/exportexcel&IDPlan='.$row['PK1'].'&IDPlanE='.$row['PK_PESTRATEGICO'].'" ><i class="icon-file"></i> Exportar</a></li>
								  <li class="divider"></li>';
								  }
								...
								$body = __toHtml($row['TITULO']).': http://www.redanahuac.mx/app/planeacion/shared.app?execute=PO51927619CDB8A/PE518EA2F51B620/PO';		
								$recurso .='  <li><a href="mailto:info@example.com?subject=\'Plan operativo: '.__toHtml($row['TITULO']).' \'&body='.$body.'" ><i class="icon-envelope"></i> Compartir</a></li>
								  <li class="divider"></li>';
								  }
								  ...
								  $recurso .='
<li><a href="?execute=planesoperativo/asignaciones&method=default&Menu=F2&SubMenu=SF21&IDPlan='.$row['PK1'].'&IDPlanE='.$row['PK_PESTRATEGICO'].'&IDJerarquia='.$row['PK_JERARQUIA'].'#&p=1&s=25&sort=1&q="><i class="icon-th-list"></i> Asignaciones</a></li>';
								...
								$recurso .='
<li><a href="?execute=planesoperativo/editestado&method=default&Menu=F2&SubMenu=SF21&IDPlan='.$row['PK1'].'&IDPlanE='.$row['PK_PESTRATEGICO'].'&IDJerarquia='.$row['PK_JERARQUIA'].'"><i class="icon-flag"></i> Cambiar estado</a></li> <li class="divider"></li>';
}
								...
								$recurso .='
<li><a href="?execute=planesoperativo/editplano&method=default&Menu=F2&SubMenu=SF21&IDPlan='.$row['PK1'].'&nodo='.$row['PK_JERARQUIA'].'"><i class="icon-pencil"></i> Editar</a></li>';
}

                				if($this->passport->getPrivilegio($row['PK1'],'P30')){
								$recurso .='<li><a href="'.$urlEditObjetivos.'"><i class=" icon-pencil"></i> Editar Objetivos</a></li>';
								}
