<?php
include "models/eventos.model.php";
...
class eventos {
	...
	function error(){
	 ...
	 $section = TEMPLATE.'modules/eventos/GRDEVENTOS.TPL';
	 $section = $this->View->loadSection($section);
	 
	// if($this->passport->privilegios->hasPrivilege('P206')){
	// $urlbtnaddplan ='<button type="button" disabled onclick="AsignarPlanEstrategico(false);" class="btn btn-small btn-warning"><i class="icon-book icon-white"></i>&nbsp;Agregar nuevo evento</button>';
	/*  }else{
	  	$urlbtnaddplan = '';
	  }*/
	...
	//	$total = $this->Model->totalnum;
		...
		if($numrecursos != 0){
			 ...
			 $recurso.='<tr>';		 	  
		 	  ...
		 	  $recurso.='<th style="border:1px #999999 solid; background:#FF4712; color:#FFF;">Nivel</th>';
...
$recurso .= '<tr>
				...
				<td style="border:1px #999999 solid; font-size:12px;">'.__toHtml($this->Model->BuscarNivel($row['PK_NIVEL']), ENT_QUOTES, "ISO-8859-1").'</td>
...
$recurso .='</tr>';
	...
	function Buscar()
	{
		...
		foreach($this->Model->usuarios as $row)
		{
			...
			$recurso .= '<tr>
				...
				<!--<td><h3>'.$row['PK_CAMPUS'].'</h3></td>-->
				<td><h3>'.__toHtml($this->Model->BuscarNivel($row['PK_NIVEL']), ENT_QUOTES, "ISO-8859-1").'</h3></td>
				<td><h3>'.__toHtml($this->Model->BuscarPrograma($row['PK_PROGRAMA']), ENT_QUOTES, "ISO-8859-1").'</h3></td>
				<td><h3>'.__toHtml($row['NOMBRE'], ENT_QUOTES, "ISO-8859-1").'</h3></td>
				<td><h3>'.utf8_encode($row['APELLIDOS']).'</h3></td>
				<td><h3>'.$row['CORREO'].'</h3></td>
				<td><h3>'.$row['TELEFONO'].'</h3></td>
				<td><h3>'.__toHtml($row['CIUDAD'], ENT_QUOTES, "ISO-8859-1").'</h3></td>
				<td><h3>'.__formatDate($row['FECHA_R']).'</h3></td>
				';
								
								
			//$recurso .='<td><a disabled class="btn btn-success btn-small" href="javascript:void(0)" onclick=""> Editar</a></td>';
					
                //	$recurso .='<td><a class="btn btn-success btn-small" href="javascript:void(0)" onclick="BuscarProspecto(\''.$row['PK1'].'\')"> Editar</a></td>';
	...
	function BuscarProspecto(){						 	
		...
		$recurso .= "#%#";
		$recurso .= __toHtml($prospecto['APELLIDOS'], ENT_QUOTES, "ISO-8859-1");
		$recurso .= "#%#";
     ...
     <div class="left">
	  ...
	  <input type="hidden" name="s" value="'.$_GET['s'].'" />';
       ...
       $paginadoHeader.='<select id="sort-menu" name="sort" onchange="Ordenar(this.value)">
		...
		$paginadoHeader.=' value="2">Ordenar por: Maestrías en línea</option>
		  <option'; if($_GET['sort']==3){$paginadoHeader .=' selected="selected" '; }
		$paginadoHeader.=' value="3">Ordenar por: Maestrías presenciales</option>
		    <option'; if($_GET['sort']==4){$paginadoHeader .=' selected="selected" '; }
	    $paginadoHeader.=' value="4">Ordenar por: Doctorados</option>
	    <option'; if($_GET['sort']==5){$paginadoHeader .=' selected="selected" '; }
	    $paginadoHeader.=' value="5">Ordenar por: Especialidades</option>
