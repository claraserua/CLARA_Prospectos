...
class uploadapoyo {
	...
	function error(){
	 ...
	 $contenido = TEMPLATE.'modules/apoyos/UPLOAD.TPL';
	 $contenido = $this->View->loadSection($contenido);
	...
	if(isset($_GET['ID'])){				
			 ...
			 $contenido =  $this->View->replace('/\#EDITAR\#/ms' ,TRUE,$contenido);
			 $contenido =  $this->View->replace('/\#IDARCHIVO\#/ms' ,$_GET['ID'],$contenido);
			 $titulo = $_GET['titulo'];
			 $descripcion = $_GET['descripcion'];
			 $autor = $_GET['autor'];
		}
	else{
		 $contenido =  $this->View->replace('/\#EDITAR\#/ms' ,FALSE,$contenido);
		
	}			  
		 
	  $contenido =  $this->View->replace('/\#TITULO\#/ms' ,__toHtml($titulo, ENT_QUOTES, "ISO-8859-1"),$contenido);
	  $contenido =  $this->View->replace('/\#DESCRIPCION\#/ms' ,__toHtml($descripcion, ENT_QUOTES, "ISO-8859-1"),$contenido);
	  $contenido =  $this->View->replace('/\#AUTOR\#/ms' ,__toHtml($autor, ENT_QUOTES, "ISO-8859-1"),$contenido); 

	   $urlcancelar = '?execute=apoyos/apoyos&method=default&Menu=F4&SubMenu=SF41#&p=1&s=25&sort=1&q=';
	   $contenido = $this->View->replace('/\#URLCANCELAR\#/ms' ,$urlcancelar,$contenido);
