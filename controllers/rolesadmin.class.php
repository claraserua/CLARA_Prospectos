<?php
include "models/rolesadmin.model.php";
...
class rolesadmin {
	...
	function __construct() {
	 $this->menu = new Menu();
	 $this->Model = new rolesadminModel();
	 ...
	 function loadContent(){
	  ...
	  $section = TEMPLATE.'modules/roles/GRDROLES.TPL';
	  $section = $this->View->loadSection($section);
	 ...
	 function Buscar(){
		...
		foreach($this->Model->roles as $row){
			 ...
			 $recurso .= '<tr>
								<td><h3>'.__toHtml($row['ROLE'], ENT_QUOTES, "ISO-8859-1").'</h3></td>
								<td><h3>'.__toHtml($row['DESCRIPCION'], ENT_QUOTES, "ISO-8859-1").'</h3></td>
								<td><h3>'.__formatDate($row['FECHA_R']).'</h3></td>
								<td align="center"><h3>'.__toHtml($row['TIPO'], ENT_QUOTES, "ISO-8859-1").'</h3></td>
								<td>
									<div class="btn-group">
