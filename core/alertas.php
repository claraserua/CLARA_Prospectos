...
class Alertas
    ...
    function getNumAlertas(){
		...
		$usuario = $_SESSION['session']['user'];
	    $sql = sprintf("SELECT * FROM NOTIFICACIONES WHERE TIPO = 'ALERT' AND PK_USUARIO = '$usuario' AND VISTO='0' ORDER BY FECHA_R");
    ...
    function getAlertas(){
	   ...
	   $alertas .='<ul id="themes" class="dropdown-menu"  style="width: 320px; height: 373px;  overflow-y: auto;">';
        ...
        //while ($r = mssql_fetch_array($result, MSSQL_ASSOC))  {
		foreach(database::getRows($sql) as $r){
			...
			if(trim($r['VISTO'])=="1"){
			  ...
			  $fecha = "<strike>".$r['FECHA_R']."</strike>";
			   $nombre  = "<strike>".__toHtml($rowu['NOMBRE'])."</strike>";
			}else{

			  $objetivo = $r['OBJETIVO'];
			  $fecha = $r['FECHA_R'];
			  $nombre  = __toHtml($rowu['NOMBRE']);
			}
