...
class Niveles
{
	...
	function initSubNiveles($padre, &$nodo) { 
	   ...
	   foreach ($rows as $row) { 
			...
			$propiedades=$this->nodeProperties;
     		$$subNode=&$nodo->addItem(new HTML_TreeNodeXL($nom,$links,$propiedades)); 
	...
	function initSubNivelesFiltro($padre, &$nodo) { 
	   ...
	   if($this->hasNivel($row['PK1']) || isset($this->passport->privilegios->roles['R00']) ){
		...
		$propiedades=$this->nodeProperties;
     	$$subNode=&$nodo->addItem(new HTML_TreeNodeXL($checkbox, "",$propiedades));      	 
	 }
...
function initNivelesMove($nivel = "0"){
	...
	function initSubNivelesMove($padre, &$nodo) { 
        ...
        $i=0; 
        while ($row = sqlsrv_fetch_array( $result, SQLSRV_FETCH_ASSOC)){//mssql_fetch_array($result, MSSQL_ASSOC)) { 
        $i++;
	    $subNode='subNode'.$i;
