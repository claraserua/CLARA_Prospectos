       ...
       // maximo por pagina
		...
		if(isset($_GET['sort'])){
			...
			switch($_GET['sort']){
				...
				break;
				case 2:
				$order = " AND PK_CAMPUS IN ('PREPA_CULIACAN')";
				break;
				case 3:
				$order = " AND PK_CAMPUS IN ('PREPA_CHIHUAHUA')";
				break;
				case 4:
				$order = " AND PK_CAMPUS IN ('PREPA_MERIDA')";
				break;	
				case 5:
				$order = " AND PK_CAMPUS IN ('PREPA_CUMBMEX')";
				break;	
				case 6:
				$order = " AND PK_CAMPUS IN ('PREPA_MADDMEX')";
				break;	
				case 7:
				$order = " AND PK_CAMPUS IN ('PREPA_OXFORD')";					
				break;
				case 8:
				$order = " AND PK_CAMPUS IN ('PREPA_MCUMBRES')";
				break;
				case 9:
				$order = " AND PK_CAMPUS IN ('PREPA_MSAGUSTIN')";
				break;	
				case 10:
				$order = " AND PK_CAMPUS IN ('PREPA_MORELIA')";
				break;	
				case 11:
				$order = " AND PK_CAMPUS IN ('PREPA_PUEBLA')";
				break;	
				case 12:
				$order = " AND PK_CAMPUS IN ('PREPA_UAQ')";
				break;	
				case 13:
				$order = " AND PK_CAMPUS IN ('PREPA_SP')";
				break;	
				case 14:
				$order = " AND PK_CAMPUS IN ('PREPA_XALAPA')";
				break;
				case 15:
				$order = " AND PK_CAMPUS IN ('PREPA_TOLUCA')";
				break;
				case 16:
				$order = " AND PK_CAMPUS IN ('PREPA_DURANGO')";
				break;
				case 17:
				$order = " AND PK_CAMPUS IN ('PREPA_OAXACA')";
				break;
...
FROM    ( SELECT ROW_NUMBER() OVER ( ORDER BY FECHA_R DESC ) AS RowNum, *
          FROM      PROSPECTOS WHERE PK_NIVEL =  AND PK_CAMPUS IN( $filter ) $order  $buscar
...
ORDER BY FECHA_R DESC";	
	  ...
	  $sql ="SELECT  P.*
FROM    ( SELECT ROW_NUMBER() OVER ( ORDER BY FECHA_R DESC ) AS RowNum, *
          FROM      PROSPECTOS WHERE PK_NIVEL = 8  $order
        ) AS P
WHERE   RowNum > $offset
    AND RowNum <= $limit 	
ORDER BY FECHA_R DESC";
		...
		$sqlcount = "SELECT * FROM PROSPECTOS WHERE PK_NIVEL = 8 $order";
		$this->totalnum = $total = database::getNumRows($sqlcount);		
	...
	function getProspectosUniversidad()
	{
		...
		function getUniversidades(){
			$sql = "SELECT * FROM UNIVERSIDADES WHERE (PK1<>'RED' AND PK1<>'J00') ORDER BY ORDEN";
			$this->universidades = database::getRows($sql);
		}
		
		function getUniversidadXID($idUniversidad){
			$sql = "
				SELECT TOP 1 DESCRIPCION FROM UNIVERSIDADES WHERE PK1='$idUniversidad'
			";
			$this->universidades = database::getRows($sql);
			foreach($this->universidades as $row){
				return $row['DESCRIPCION'];
			}
			return "";
		}
		
		function getProspectos($fech_1,$fech_2)
		{
			if ($fech_1!='' && $fech_2!='')
				$condicion="WHERE (CAST(FECHA_R AS DATE)>='$fech_1' AND CAST(FECHA_R AS DATE)<='$fech_2')";
			else
				$condicion="";
			$sql = "
				SELECT PK_CAMPUS, CAST(FECHA_R AS DATE) AS 'FECHA' FROM PROSPECTOS $condicion
				ORDER BY FECHA_R ASC
			";
			$this->prospectos = database::getRows($sql);	
		}
		
		function getProspectosXUniversidad($idUniversidad,$fech_1,$fech_2)
		{
			if($fech_1!='' && $fech_2!='')
				$condicion="AND (CAST(FECHA_R AS DATE)>='$fech_1' AND CAST(FECHA_R AS DATE)<='$fech_2')";
			else
				$condicion="";
			$sql = "
				SELECT PK_CAMPUS, CAST(FECHA_R AS DATE) AS 'FECHA' FROM PROSPECTOS
				WHERE PK_CAMPUS='$idUniversidad' $condicion
				ORDER BY FECHA_R ASC
			";
			$this->fordebug = $sql;
			$this->prospectos = database::getRows($sql);	
		...
		//*
		function getProspectos_2($fech_1,$fech_2)
		{
			if ($fech_1!='' && $fech_2!='')
				$condicion="AND (CAST(FECHA_R AS DATE)>='$fech_1' AND CAST(FECHA_R AS DATE)<='$fech_2')";
			else
				$condicion="";
			$sql = "
				SELECT PK_CAMPUS, CAST(FECHA_R AS DATE) AS 'FECHA' FROM PROSPECTOS WHERE PK_CAMPUS<>'' $condicion
				ORDER BY PK_CAMPUS ASC
			";
			$this->fordebug = $sql;
			$this->prospectos = database::getRows($sql);	
		}
		...
		{
			$sql = "SELECT * FROM DIVISIONES WHERE PK_NIVEL = '2' ORDER BY ORDEN";
			$this->divisiones = database::getRows($sql);
		}
