...
class presentacionProyectoModel {
	...
	/*	if(isset($_GET['idLineaE'])){
		...
		function getEstadoProyecto($id){
		    ...
		    switch(trim($row['ESTADO'])){
				...
				case "R":
					$estado = '<span class="label label-warning">Revision</span>';
			  		break;
...
ORDER BY $order";
	...
	function EnviarResolucionFavorable($idProyecto,$estado){
		...
		foreach(database::getRows($sql) as $row){
					 ...
					 if($passport->getPrivilegioRol($row['ROL'],'P316')){

								//   if($passport->getPrivilegioRol($row['ROL'],'P80')){
					$this->EnviarCorreoResolucionFavorable($_SESSION['session']['user'],$row['PK_USUARIO'],$idProyecto);
					                //}
	...
	function EnviarCorreoResolucionFavorable($de,$para,$idProyecto){
    ...
    $rowpara = database::getRow($sql);
                ...
                $sql = "SELECT PROYECTO, FOLIO FROM PROYECTOS WHERE PK1 = '$idProyecto'";
...
$mail->FromName = 'Sistema de Gestión de Propuesta de la RUA';
...
$mail->Subject = 'Inicio de Proyecto';
...
<title>Sistema de Gesti&oacute;n de Propuesta de la RUA</title>
...
<h1 style="font-family:Segoe UI, Tahoma, sans-serif; margin:0px; font-size:12pt; line-height:19px; color:#072B60; font-weight:normal;color:#ffffff;">Se ha dado resoluci&oacute;n favorable a la propuesta presentada</br></h1>
...
<p style="font-family:Segoe UI, Tahoma, sans-serif; font-size:10pt; color:#000;"> La propuesta: <strong>'.$proyecto.'</strong> con folio: '.$folio.' ha sido aprobada por el Comit&eacute; de Direcci&oacute;n del Cosejo Superior de la RUA</p>

 <p style="font-family:Segoe UI, Tahoma, sans-serif; font-size:10pt; color:#000;">Contar&aacute; con el apoyo del Coordinador de Desarrollo de la Secretar&iacute;a Ejecutiva de la RUA</p>
...
//}
 ...
 function EnviarResolucionFavorableAjustes($idProyecto,$estado){
		...
		//while ($row = mssql_fetch_array($result2, MSSQL_ASSOC)) {
		foreach(database::getRows($sql) as $row){

					 $passport = new Authentication();

					 if($passport->getPrivilegioRol($row['ROL'],'P317')){

								//   if($passport->getPrivilegioRol($row['ROL'],'P80')){
					$p=$this->EnviarCorreoResolucionFavorableAjustes($_SESSION['session']['user'],$row['PK_USUARIO'],$idProyecto);
		...
		function EnviarCorreoResolucionFavorableAjustes($de,$para,$idProyecto){
...
$mail->Subject = 'Propuesta en ajustes';
...
<h1 style="font-family:Segoe UI, Tahoma, sans-serif; margin:0px; font-size:12pt; line-height:19px; color:#072B60; font-weight:normal;color:#ffffff;">Se ha enviado una propuesta con petici&oacute;n de ajustes</h1>

<p style="margin:0;font-size:11pt;font-family:Segoe UI, Tahoma, sans-serif;color:#000;color:#ffffff;">Ahora Puede ingresar, revisar los comentarios y ajustar la propuesta</p>
...
<p style="font-family:Segoe UI, Tahoma, sans-serif; font-size:10pt; color:#000;"> La propuesta: <strong>'.$proyecto.'</strong>&nbsp;con folio: '.$folio.' ha sido revisada por el usuario: </p>
...
<div style="margin-top:20px;font-family:\'Segoe UI\', Tahoma, sans-serif; font-size:10pt; line-height:13pt; color:#000;">
                  ...
                  <ul style="font-family:Segoe UI, Tahoma, sans-serif;color:#000;font-size:10pt;">
                    ...
                    <li>Debe ajustar la propuesta seg&uacute;n los comentarios de los expertos y del Coordinador de Desarrollo.</li>



                    <li>S&oacute;lo los usuarios que tengan permiso podr&aacute;n enviar la propuesta ajustada.</li>
...
<br><p style="font-family:Segoe UI, Tahoma, sans-serif; font-size:10pt; color:#000;">Por favor vaya a la p&aacute;gina de inicio de sesi&oacute;n del Sistema de Gesti&oacute;n de Proyectos e ingrese con su usuario y contrase&ntilde;a:  </p>
...
//
 ...
 function EnviarResolucionNoFavorable($idProyecto,$estado){
		...
		foreach(database::getRows($sql) as $row){
					 ...
					 if($passport->getPrivilegioRol($row['ROL'],'P318')){

								//   if($passport->getPrivilegioRol($row['ROL'],'P80')){
					$p=$this->EnviarCorreoResolucionNoFavorable($_SESSION['session']['user'],$row['PK_USUARIO'],$idProyecto);
		...
		function EnviarCorreoResolucionNoFavorable($de,$para,$idProyecto){
...
$mail->Subject = 'Propuesta no aprobada';
...
<h1 style="font-family:Segoe UI, Tahoma, sans-serif; margin:0px; font-size:12pt; line-height:19px; color:#072B60; font-weight:normal;color:#ffffff;">Se ha dado resoluci&oacute;n no favorable a la propuesta presentada</h1>
...
<p style="font-family:Segoe UI, Tahoma, sans-serif; font-size:10pt; color:#000;"> La propuesta: <strong>'.$proyecto.'</strong>&nbsp;no ha sido aprobada por el Comit&eacute; de Direcci&oacute;n de Consejo Superior de la RUA.</p>


<p style="font-family:Segoe UI, Tahoma, sans-serif; font-size:10pt; color:#000;">Para cualquier aclaraci&oacute;n o duda, puede contactar al Coordinador de Desarrollo de la Secretar&iacute;a Ejecutiva de la RUA.</p>
