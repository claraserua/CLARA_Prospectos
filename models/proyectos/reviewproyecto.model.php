...
class reviewProyectoModel {
 ...
 //para el id de inversion guardar en la tabla proyectos por si se guardaron datos vacios en PK_FINVERSION_INTERNA
	...
	ORDER BY $order";
		...
		$sqlcount = "SELECT PK1 FROM PL_POPERATIVOS WHERE	PK_PESTRATEGICO = '$idPlanE' $filter ";
	...
	function RegresarPropUni($idProyecto,$estado){
		...
		foreach(database::getRows($sql) as $row){
					 ...
					 if($passport->getPrivilegioRol($row['ROL'],'P300')){
	...
	function EnviarCorreo($de,$para,$idProyecto){
...
$mail->Subject = 'Propuesta enviada a elaboración';
...
<title>Sistema de Gesti&acute;n de Proyectos RUA</title>
...
<h1 style="font-family:Segoe UI, Tahoma, sans-serif; margin:0px; font-size:12pt; line-height:19px; color:#072B60; font-weight:normal;color:#ffffff;">Se ha enviado una propusta para elaboraci&oacute;n.</br> Puede ingresar, revisar los comentarios</h1>

<p style="margin:0;font-size:11pt;font-family:Segoe UI, Tahoma, sans-serif;color:#000;color:#ffffff;">y añadir la informaci&oacute;n solicitada</p>
...
<p style="font-family:Segoe UI, Tahoma, sans-serif; font-size:10pt; color:#000;"> La propuesta: <strong>'.$proyecto.'</strong>&nbsp;necesita informaci&oacute;n adicional, ha sido enviado por el usuario:</p>
...
<div style="margin-top:20px;font-family:\'Segoe UI\', Tahoma, sans-serif; font-size:10pt; line-height:13pt; color:#000;">
				  ...
				  <ul style="font-family:Segoe UI, Tahoma, sans-serif;color:#000;font-size:10pt;">
					...
					<li>Puede consultar la Gu&iacute;a de Presentaci&oacute;n de Propuesta de la RUA para verificar el contenido de la propuesta.</li>
<li>Debe completar la informaci&oacute;n seg&uacute;n los comentarios del Coordinador de Planeci&oacute;n.</li>


					<li>S&oacute;lo los usuarios que tengan permiso podr&aacute;n enviar la propuesta completa.</li>
...
<br><p style="font-family:Segoe UI, Tahoma, sans-serif; font-size:10pt; color:#000;">Por favor vaya a la p&aacute;gina de inicio de sesi&oacute;n del Sistema de Gesti&oacute;n de propuestas e ingrese con su usuario y contrase&ntilde;a:	 </p>
...
//}
 ...
 function EnviarConRevisionPreliminar($idProyecto,$estado){
		...
		foreach(database::getRows($sql) as $row){
					 ...
					 if($passport->getPrivilegioRol($row['ROL'],'P303')){

								//	 if($passport->getPrivilegioRol($row['ROL'],'P80')){
					$this->EnviarCorreoRevisionPreliminar($_SESSION['session']['user'],$row['PK_USUARIO'],$idProyecto);
	...
	function EnviarCorreoRevisionPreliminar($de,$para,$idProyecto){
...
$mail->Subject =  'Propuesta para revisión';
...
<h1 style="font-family:Segoe UI, Tahoma, sans-serif; margin:0px; font-size:12pt; line-height:19px; color:#072B60; font-weight:normal;color:#ffffff;">Se ha enviado la propuesta para revisi&oacute;n</h1>

<p style="margin:0;font-size:11pt;font-family:Segoe UI, Tahoma, sans-serif;color:#000;color:#ffffff;">Puede ingresar y revisar la prupuesta</p>
...
<p style="font-family:Segoe UI, Tahoma, sans-serif; font-size:10pt; color:#000;"> La propuesta: <strong>'.$proyecto.'</strong> ha sido enviada por el usuario:	</p>
...
<div style="margin-top:20px;font-family:\'Segoe UI\', Tahoma, sans-serif; font-size:10pt; line-height:13pt; color:#000;">
				  ...
				  <ul style="font-family:Segoe UI, Tahoma, sans-serif;color:#000;font-size:10pt;">
					...
					<li>Puede ingresar al sistema y enviar la propuesta a expertos.</li>



					<li>S&oacute;lo los usuarios que tengan permiso podr&aacute;n enviar la propuesta para revisi&oacute;n expertos.</li>
...
//}
	...
	function eliminarEtapa($id){
		...
		$sql = "SELECT PK1 FROM PASOS WHERE PK_ETAPA = '$id'";
		//$result = database::executeQuery($sql);
		...
		$sql = "DELETE FROM ETAPAS WHERE PK1 = '$id'";
		$result = database::executeQuery($sql);

		$sql = "DELETE FROM PASOS WHERE PK_ETAPA = '$id'";
		$result = database::executeQuery($sql);
		...
		ECHO $ids;
			...
			function updateEtapa($id,$etapa){

				$sql = "UPDATE ETAPAS SET ETAPA = '$etapa' WHERE PK1=$id";
				$result = database::executeQuery($sql);
		...
		function getPasos($id){
		 ...
		 $row = database::getRow($sql);
			...
			function eliminarPaso($id){
			   ...
			   $sql = "DELETE FROM PASOS WHERE PK1 = '$id'";
			   $result = database::executeQuery($sql);
			...
			function updatePaso($id,$paso,$finicio,$ffinal){

				$sql = "UPDATE PASOS SET PASO = '$paso' , F_INICIO= '$finicio', F_TERMINO = '$ffinal' WHERE PK1 = $id";
				$result = database::executeQuery($sql);

			}


			function updatePasoDate($id,$finicio,$ffinal){


				$sql = "UPDATE PASOS SET  F_INICIO= '$finicio', F_TERMINO = '$ffinal' WHERE PK1 = $id";
				$result = database::executeQuery($sql);
			...
			function updateGantt($id,$finicio,$ffinal){

				$sql = "UPDATE proyectos SET  FECHA_I= '$finicio', FECHA_T = '$ffinal' WHERE PK1 ='$id'";
				$result = database::executeQuery($sql);
