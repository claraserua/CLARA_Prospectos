...
class revisadoProyectoModel {
 ...
 //para el id de inversion guardar en la tabla proyectos por si se guardaron datos vacios en PK_FINVERSION_INTERNA
	...
	function EnviarAjustesUniversidad($idProyecto,$estado){
		...
		foreach(database::getRows($sql) as $row){
					 ...
					 if($passport->getPrivilegioRol($row['ROL'],'P309')){

								//   if($passport->getPrivilegioRol($row['ROL'],'P80')){
					$this->EnviarCorreoAjustesUniversidad($_SESSION['session']['user'],$row['PK_USUARIO'],$idProyecto);
					                //}
	...
	function EnviarCorreoAjustesUniversidad($de,$para,$idProyecto){
...
//}
	...
	function IntegrarPropuesta($idProyecto,$estado){
		...
		foreach(database::getRows($sql) as $row){
					 ...
					 if($passport->getPrivilegioRol($row['ROL'],'P311')){
...
$mail->Subject =  'Se ha enviado una propuesta revisada por expertos';
