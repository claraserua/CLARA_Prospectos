...
ORDER BY $order";
	...
	function EnviarPresentacionPropuesta($idProyecto,$estado){
	    ...
	    foreach(database::getRows($sql) as $row){ //while ($row = mssql_fetch_array($result, MSSQL_ASSOC)) {
					 ...
					 if($passport->getPrivilegioRol($row['ROL'],'P337')){

								//   if($passport->getPrivilegioRol($row['ROL'],'P80')){
					$this->EnviarCorreoPresentacionProp($_SESSION['session']['user'],$row['PK_USUARIO'],$idProyecto);
					                //}
	...
	function EnviarCorreoPresentacionProp($de,$para,$idProyecto){
...
$mail->Subject =  'Sesión agendada para presentación de propuesta';
...
<h1 style="font-family:Segoe UI, Tahoma, sans-serif; margin:0px; font-size:12pt; line-height:19px; color:#072B60; font-weight:normal;color:#ffffff;">Se ha agendado sesi&oacute;n para presentaci&oacute;n de propuesta</h1>
...
<p style="font-family:Segoe UI, Tahoma, sans-serif; font-size:10pt; color:#000;">La sesi&oacute;n para presentaci&oacute;n de la propuesta: <strong>'.$proyecto.'</strong> ha sido agendada por el usuario: </p>
