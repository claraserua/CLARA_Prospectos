...
FROM    ( SELECT ROW_NUMBER() OVER ( ORDER BY FECHA_R DESC ) AS RowNum, *
          FROM      PROSPECTOS WHERE  $order $buscar
...
ORDER BY FECHA_R DESC";
		...
		$sqlcount = "SELECT * FROM PROSPECTOS WHERE $order $buscar";
    ...
    function ObtenerUsuarios($ids){
	 ...
	 function RolResponsable($responsable,$rol){
		...
		function Enviar(){	
	        ...
	        foreach($this->usuarios as $idprospecto){	 		
					...
					$sql = "SELECT PK_CAMPUS,PK_NIVEL FROM PROSPECTOS WHERE PK1='$idprospecto'";   
				    $usr = database::getRow($sql);				    
				    $campus = trim($usr['PK_CAMPUS']);	
				    $nivel = trim($usr['PK_NIVEL']);
				    
				    $rol = '';
					if($nivel == 3){ $rol = 'R56B4F9D5148FE'; }//ME
		         ...
		         if($numresp != 0){	  		
					 ...
					 {				
						 if($responsable['PK1']!='admin'){	
					    	...
					    	$trol =	$this->RolResponsable($responsable['PK1'],$rol); 			                        
